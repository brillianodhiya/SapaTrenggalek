...
export default function DatabaseManager() {
  ...
  const cleanupDatabase = async () => {
    if (!confirm('Apakah Anda yakin ingin membersihkan data lama dan test data?')) {
      return
    }
    ...
    try {
      ...
      if (data.success) {
        setMessage(`✅ Cleanup berhasil! ${data.cleanup_summary.total_deleted} entries dihapus`)
  ...
  const resetDatabase = async () => {
    const confirmText = prompt('PERINGATAN: Ini akan menghapus SEMUA data!\nKetik "DELETE ALL" untuk konfirmasi:')
    ...
    try {
      ...
      if (data.success) {
        setMessage(`🚨 Database reset berhasil! ${data.reset_summary.entries_deleted} entries dihapus`)
        fetchStats() // Refresh stats
      } else {
          ...
          {stats && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              ...
              <div className="bg-green-50 p-4 rounded-lg">
                <div className="text-2xl font-bold text-green-600">{stats.real_data_entries}</div>
                <div className="text-sm text-green-600">Real Data</div>
              </div>
              <div className="bg-yellow-50 p-4 rounded-lg">
                <div className="text-2xl font-bold text-yellow-600">{stats.test_data_entries}</div>
        ...
        <div className="mt-4 p-3 bg-gray-50 rounded-lg">
          <div className="flex items-start">
            ...
            <div className="text-sm text-gray-600">
              <strong>Peringatan:</strong> Cleanup akan menghapus data lama (>1 jam) dan test data. 
              Reset akan menghapus SEMUA data. Gunakan dengan hati-hati!
            </div>
          </div>
        </div>
      </div>
