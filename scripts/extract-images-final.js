...
// Enhanced image extraction that works with various URL types
...
function generateContextualPlaceholder(content, category = "berita") {
  const keywords = extractKeywords(content);
  const primaryKeyword = keywords[0] || "trenggalek";

  // Use Unsplash with relevant search terms
  const searchTerms = {
    berita: `news,newspaper,${primaryKeyword}`,
    laporan: `report,document,${primaryKeyword}`,
    aspirasi: `community,people,${primaryKeyword}`,
    lainnya: `trenggalek,indonesia,${primaryKeyword}`,
  };

  const searchTerm = searchTerms[category] || searchTerms.lainnya;
  const width = 400;
  const height = 250;

  return `https://images.unsplash.com/${width}x${height}/?${encodeURIComponent(
    searchTerm
  )}&sig=${Math.random()}`;
}

function extractKeywords(content) {
  // Simple keyword extraction from Indonesian content
  const words = content
    .toLowerCase()
    .replace(/[^\w\s]/g, " ")
    .split(/\s+/)
  ...
  // Common Indonesian words to filter out
  const stopWords = [
    "yang",
    "untuk",
    "dari",
  ...
  try {
    ...
    for (const entry of entries) {
      try {
        ...
        const extractedImage = await extractImageFromContent(
          entry.source_url || "",
          entry.content
        );
        if (extractedImage && (await isValidImageUrl(extractedImage.src))) {
          imageUrl = extractedImage.src;
          imageAlt = extractedImage.alt;
          imageSource = "content";
        }

        // Method 2: If no image found, generate contextual placeholder
        if (!imageUrl) {
          imageUrl = generateContextualPlaceholder(
            entry.content,
            entry.category
          );
